import { Shirt, ShirtFilterState, ShirtSortOption } from '../types/product.js';

/**
 * CENTRALIZED MENSWEAR SHIRT DATASET (50 PRODUCTS)
 * Features 100% REAL HIGH-QUALITY PRODUCT IMAGES with working filters.
 */
export const DEVELOPMENT_SHIRTS: Shirt[] = [
  {
    "id": "shirt-001",
...
export function queryShirts(
  filters: ShirtFilterState = { fits: [], fabrics: [], sizes: [], colors: [], sleeves: [], patterns: [] },
  sort: ShirtSortOption = 'featured'
): Shirt[] {
  ...
  if (filters.searchQuery && filters.searchQuery.trim() !== '') {
    ...
    result = result.filter(
      shirt =>
        shirt.name.toLowerCase().includes(q) ||
        shirt.description.toLowerCase().includes(q) ||
        shirt.fabric.toLowerCase().includes(q) ||
        shirt.fit.toLowerCase().includes(q) ||
        shirt.collar.toLowerCase().includes(q) ||
        shirt.pattern.toLowerCase().includes(q)
    );
  }

  // 2. Fits
  if (filters.fits && filters.fits.length > 0) {
    result = result.filter(shirt => filters.fits.includes(shirt.fit));
  }

  // 3. Fabrics
  if (filters.fabrics && filters.fabrics.length > 0) {
    result = result.filter(shirt => filters.fabrics.includes(shirt.fabric));
  }

  // 4. Sizes
  if (filters.sizes && filters.sizes.length > 0) {
    result = result.filter(shirt => shirt.sizes.some(s => filters.sizes.includes(s)));
  }

  // 5. Colors
  if (filters.colors && filters.colors.length > 0) {
    result = result.filter(shirt => shirt.colors.some(c => filters.colors.includes(c.name)));
  }

  // 6. Sleeves
  if (filters.sleeves && filters.sleeves.length > 0) {
    result = result.filter(shirt => filters.sleeves.includes(shirt.sleeve));
  }

  // 7. Patterns
  if (filters.patterns && filters.patterns.length > 0) {
    result = result.filter(shirt => filters.patterns.includes(shirt.pattern));
  }

  // 8. Price Range
  if (filters.minPrice !== undefined) {
    result = result.filter(shirt => shirt.price >= filters.minPrice!);
  }
  if (filters.maxPrice !== undefined) {
    result = result.filter(shirt => shirt.price <= filters.maxPrice!);
  }

  // 9. Ratings
  if (filters.minRating !== undefined) {
    result = result.filter(shirt => shirt.rating >= filters.minRating!);
  }

  // 10. Deals Only
  if (filters.onlyDeals) {
    result = result.filter(shirt => shirt.isDeal);
  }

  // 11. New Arrivals Only
  if (filters.onlyNewArrivals) {
    result = result.filter(shirt => shirt.isNewArrival);
  }
...
export function getShirtById(id: string): Shirt | undefined {
  return DEVELOPMENT_SHIRTS.find(s => s.id === id || s.slug === id);
}

export function getRelatedShirts(currentShirtId: string, limit = 4): Shirt[] {
  const current = getShirtById(currentShirtId);
  if (!current) return DEVELOPMENT_SHIRTS.slice(0, limit);

  return DEVELOPMENT_SHIRTS.filter(
    s => s.id !== current.id && (s.fit === current.fit || s.fabric === current.fabric)
  ).slice(0, limit);
}
