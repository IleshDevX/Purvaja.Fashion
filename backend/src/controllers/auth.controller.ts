...
import { body, forgotSchema, loginSchema, registerSchema, resetSchema, updateMeSchema, verifyEmailSchema } from '../validators/auth.validator.js';
...
export const verifyEmail: RequestHandler = async (req, res, next) => { try {
  const input = body(verifyEmailSchema, req.body);
  if ('otp' in input) {
    const result = await auth.verifyRegistrationOtp(input.email, input.otp);
    const csrfToken = establishSession(res, result.sessionToken, result.maxAgeMs);
    res.json({ success: true, data: { user: result.user, csrfToken } });
    return;
  }
  await auth.verify(input.token);
...
export const resendVerification: RequestHandler = async (req, res, next) => { try { await auth.resendVerification(body(forgotSchema, req.body).email); res.json({ success: true, data: { message: 'If an account exists, a verification code will be sent.' } }); } catch (error) { next(error); } };
